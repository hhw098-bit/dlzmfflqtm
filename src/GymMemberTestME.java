...
//클래스메소드 선언
	...
	*/	
	void usePT() {
		...
		if(ptCount>0) {
			...
			ptCount--; //-=1; @Q
		//진행메세지 출력
			System.out.println(name + "님 PT 진행! 남은 횟수: "+ptCount+"회 입니다." );
			
		}else {// 남은 횟수가 0인 경우 검사
	...
	*/
	void chargePT(int n) {
		//기존 PT횟수에 충전할 횟수 n을 더함
		ptCount += n;  
		//충전 횟수와 총 잔여 횟수를 출력
	 System.out.println( name + "님 PT"+ n
			 +"회 충전 완료. 총 잔여: "+ ptCount +"회");
	...
	*/
	...
	//3단계  :  객체 생성후 데이터 저장 및 사용 
		GymMemberTestME guest = new GymMemberTestME();
